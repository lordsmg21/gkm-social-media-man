...
export function NotificationCenter({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      <div className="fixed right-0 top-0 h-screen w-full max-w-md sm:max-w-lg glass-modal shadow-2xl border-l border-border overflow-hidden flex flex-col" style={{
        background: 'rgba(255, 255, 255, 0.95)',
        backdropFilter: 'blur(20px)',
        border: '1px solid rgba(234, 179, 8, 0.2)',
      }}>
        {/* Header */}
        <div className="p-4 sm:p-6 border-b border-border" style={{
          ...
          <div className="h-full p-4 pb-6 space-y-4 overflow-y-auto">
            {notifications.length === 0 ? (
              ...
              notifications.map((notification) => (
                <div 
                  key={notification.id}
                  className={`glass-card p-4 rounded-lg border transition-all hover:shadow-md ${
                    !notification.read 
